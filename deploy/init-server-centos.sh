#!/bin/bash
# Sky-Server 服务器初始化脚本 - CentOS/RHEL 版本
# 执行此脚本会自动安装所有依赖: MySQL 8.0, Redis, Go, sky-server

set -e

echo "=========================================="
echo "   Sky-Server 生产环境初始化 (CentOS)"
echo "=========================================="
echo ""
...
# 安装基础工具
echo ">>> [2/8] 安装基础工具..."
yum install -y wget curl git vim net-tools nginx && yum groupinstall -y "Development Tools"
...
if [ -n "$TEMP_PASSWORD" ]; then
    mysql -uroot -p"$TEMP_PASSWORD" -e "ALTER USER 'root'@'localhost' IDENTIFIED BY 'SkyServer@123';"
fi
mysql -uroot -p"SkyServer@123" -e "CREATE DATABASE IF NOT EXISTS sky_server DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"
echo "数据库 sky_server 创建完成"
echo "MySQL root password: SkyServer@123"

# 下载 SQL 初始化脚本
echo ">>> [7/8] 下载并导入初始化 SQL..."
cd /tmp
wget https://raw.githubusercontent.com/sky-xhsoft/sky-server/master/sqls/create_skyserver.sql
mysql -uroot -p"SkyServer@123" sky_server < /tmp/create_skyserver.sql
echo "数据库初始化完成"
...
# 修改数据库配置为新密码
sed -i 's/password: .*/password: "SkyServer@123"/g' /opt/sky-server/configs/config.yaml
