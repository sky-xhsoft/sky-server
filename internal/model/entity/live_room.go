...
type LiveRoom struct {
	...
	RoomType         string     `gorm:"column:ROOM_TYPE;type:varchar(50);default:'video';index" json:"roomType"`               // 直播间类型：video(视频直播), image(图片直播), vr(VR直播), audio(语音直播), graphic(图文直播)
	...
	RoomStage        string     `gorm:"column:ROOM_STAGE;type:varchar(50);default:'formal'" json:"roomStage"`                  // 直播间阶段：formal(正式直播), test(测试直播)
	...
	ViewingMethod    string     `gorm:"column:VIEWING_METHOD;type:varchar(50);default:'public'" json:"viewingMethod"`          // 观看方式：public(公开), encrypted(加密), paid(付费), ticket(购票进入), enterprise(企业成员观看), custom(自建成员观看)
	...
	PlaybackMethod   string     `gorm:"column:PLAYBACK_METHOD;type:varchar(50);default:'post_end'" json:"playbackMethod"`      // 回放方式：post_end(结束后回放), real_time(实时回放), no_playback(结束后不回放)
